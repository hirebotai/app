    ...
    for attempt in (1, 2):
        try:
            resp = requests.post("https://api.groq.com/openai/v1/chat/completions",
                                 headers=headers, json=payload, timeout=timeout, verify=False)
...
def transcribe_audio(keys, audio_data, timeout=12):
    """STT: Groq Whisper first, Google fallback. Returns (text, provider) or (None, None)."""
    import requests
    key = (keys or {}).get("groq") or ""
    if key.strip():
        try:
            ...
            print(f"[STT] Groq failed, falling back to Google: {redact(e)}")
    try:
        text = audio_data.recognize_google()
        if text and text.strip():
            return text.strip(), "google"
    except Exception as e:
        if type(e).__name__ != "UnknownValueError":
            print(f"[STT] Google failed: {redact(e)}")
    return None, None
