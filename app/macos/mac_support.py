...
def get_selected_text():
    """Return the currently selected text in the focused app window.

    Uses the macOS Accessibility API (AXUIElement). Requires the app to hold
    Accessibility permission. Returns None when nothing is selected or the API
    is unavailable.
    """
    try:
        import ApplicationServices
    except ImportError:
        print("[mac_support] ApplicationServices not available")
        return None
    try:
        system_wide = ApplicationServices.AXUIElementCreateSystemWide()
        focused_app = ApplicationServices.AXUIElementCopyAttributeValue(
            system_wide,
        ...
        if not focused_app:
            return None
        focused_element = ApplicationServices.AXUIElementCopyAttributeValue(
            focused_app,
        ...
        if not focused_element:
            return None
        selected = ApplicationServices.AXUIElementCopyAttributeValue(
            focused_element,
        ...
        print(f"[mac_support] get_selected_text failed: {e}")
    return None
