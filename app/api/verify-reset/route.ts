...
import { z } from 'zod';
import { verifyOtpToken, createResetToken } from '@/lib/otp';
...
export async function POST(request: NextRequest) {
  ...
  const token = request.cookies.get('hb_reset')?.value;
  if (!token) {
    return NextResponse.json(
      { error: 'No reset code was issued. Please request a new one.' },
      { status: 400 }
    );
  }
  ...
  response.cookies.set('hb_reset_ok', resetToken, {
    httpOnly: true,
    sameSite: 'lax',
    secure: process.env.NODE_ENV === 'production',
    path: '/',
