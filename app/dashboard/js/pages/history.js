...
const HistoryPage = {
  ...
  async selectSession(sessionId) {
    ...
    // Load logs (filtered to the active category so only the right session's
    // entries appear, e.g. when the same session id has screen + practice logs).
    try {
      const logs = await Api.getSessionLogs(sessionId, Store.get('historyCategory'));
