...
const ApiKeysPage = {
  ...
  renderLicense(settings) {
    ...
    // When a previously accepted key is rejected by the server (expired,
    // revoked, not found, bound elsewhere), the app clears it but records why
    // in license_state so we can say "your license expired" instead of the
    // misleading "trial ended" message.
    const rejectedState = settings.license_state || '';
    const REJECTED_COPY = {
      expired: ['License Expired', 'Your license key has expired or been revoked. Reactivate it or enter a new key.'],
      notfound: ['License No Longer Valid', 'This license key is no longer valid on our server. Enter a new key.'],
      bound: ['License Bound Elsewhere', 'This license key is already bound to another computer. Contact support to transfer it.'],
      invalid: ['License Rejected', 'The server rejected this license key. Double-check it and try again.']
    };
    ...
    } else if (REJECTED_COPY[rejectedState]) {
      badge.textContent = REJECTED_COPY[rejectedState][0];
      badge.className = 'license-badge';
      if (status) {
        status.textContent = REJECTED_COPY[rejectedState][1];
