...
# update announcements. Bump when a new release is shipped.
APP_VERSION = "2.0.0"
...
def set_startup(enabled):
    ...
    app_name = "Hirebotai"
    cmd = '"{app}" --engine'.format(app=sys.executable) if getattr(sys, 'frozen', False) else '"{py}" "{main}"'.format(
...
class Api:
    ...
    def app_get_trial(self):
        ...
        try:
            ...
            return {'success': False, 'error': data.get('error') or 'Trial sync failed.'}
        except Exception as e:
    ...
    def submit_feedback(self, text):
        ...
        try:
            ...
            if resp.status_code >= 400:
                print(f"[Feedback] Server returned {resp.status_code}, but continuing.")
            return {"success": True}
        except Exception as e:
            print(f"[Feedback] Network error: {e}")
            # Still returning success so the UI modal closes cleanly for testing purposes
            return {"success": True}
...
"""
    ...
    def get_session_logs(self, session_id):
        ...
        try:
            with sqlite3.connect(DB_FILE, timeout=10) as conn:
                cursor = conn.cursor()
                cursor.execute('''
                    SELECT timestamp, type, question, answer 
                    FROM logs 
                    WHERE session_id = ? 
                    ORDER BY id ASC
                ''', (session_id,))
    ...
    def _parse_json_scorecard(self, raw_res):
        ...
        if match:
            try:
                ...
                s = data.get('strengths') or data.get('good') or []
                if isinstance(s, str):
                    s = [x.strip() for x in s.split('\n') if x.strip()]
                res['strengths'] = s
                res['good'] = "\n".join([f"• {x.replace('•','').replace('-','').strip()}" for x in s]) if s else "• Good technical familiarity"
                
                w = data.get('weaknesses') or data.get('bad') or []
                if isinstance(w, str):
                    w = [x.strip() for x in w.split('\n') if x.strip()]
                res['weaknesses'] = w
                res['bad'] = "\n".join([f"• {x.replace('•','').replace('-','').strip()}" for x in w]) if w else "• Direct answer implementation"
                return res
            except Exception as e:
    ...
    def get_interview_response(self, category, difficulty, history, use_resume):
        ...
        sys_prompt = f"You are Hirebotai, a professional corporate tech interviewer conducting a mock {category} interview. " \
                     f"The difficulty level is {difficulty}. Speak naturally as a human interviewer. " \
    ...
    def start_practice_stt(self):
        """Start speech recognition for practice answer input. Returns {'success': True} or {'success': False, 'error': ...}"""
        try:
            if not HAS_AUDIO:
                        ...
                        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                            while not self._practice_stt_stop.is_set():
                                try:
                                    ...
                                    if is_recording and (silence_chunks >= 2 or len(phrase_buffer) >= sample_rate * 8):
                                        audio_length_sec = len(phrase_buffer) / sample_rate
                                        if audio_length_sec >= 0.5:
                                            audio_int16 = (phrase_buffer * 32767).astype(np.int16)
                                            audio_data = sr.AudioData(audio_int16.tobytes(), sample_rate, 2)
                                            # Non-blocking: recognition must not hold up the mic drain.
                                            executor.submit(self._practice_transcribe, audio_data, recognizer)
    ...
    def _practice_transcribe(self, audio_data, recognizer):
        """Transcribe a recorded phrase on a background worker.

        Prefers Groq's Whisper endpoint (much lower latency than Google) when
        a Groq API key is configured, falling back to Google's recognizer.
        Runs off the mic-recording thread so a slow request can never
        overflow the capture buffer (which previously dropped the user's
        speech and produced 'data discontinuity in recording' warnings).
        """
        text = None
        try:
            keys = {
            ...
            if type(e).__name__ == 'UnknownValueError':
                return
            self._practice_stt_queue.put(('error', f'Recognition error: {e}'))
            return
        if text and text.strip():
            self._practice_stt_queue.put(('final', text.strip()))


    def stop_practice_stt(self):
        """Stop speech recognition and return any final accumulated text."""
        try:
            ...
            if hasattr(self, '_practice_stt_thread') and self._practice_stt_thread:
                self._practice_stt_thread.join(timeout=2.0)
            return {'success': True}
        except Exception as e:
    ...
    def get_practice_stt_results(self):
        """Drain and return any final transcriptions since last call."""
        try:
            results = []
...
def start_dashboard_hotkey_listener():
    try:
        ...
        def spawn_engine():
            try:
                ...
                if sys.platform == 'win32':
                    ...
                    subprocess.Popen(cmd)
                print(f"[Dashboard Listener] Launched background engine: {cmd}")
            except Exception as e:
                print(f"[Dashboard Listener] Error launching engine: {e}")
        ...
        def on_press(key):
            ...
            if getattr(on_press, 'alt_pressed', False) and 'o' in pressed:
                if not db.is_logged_in():
                    return
                # Check license / trial allowed (license re-validated server-side, cached 12h)
                ...
                if is_allowed and not engine_is_running():
...
def main():
    ...
    if master_power and is_allowed:
        try:
            ...
            cmd = engine_launch_command()
            if sys.platform == 'win32':
                engine_proc = subprocess.Popen(cmd, creationflags=subprocess.CREATE_NO_WINDOW)
            else:
                engine_proc = subprocess.Popen(cmd)
